...
namespace Oleg_ivo.Base.WPF.Converters
{
    public class MultiBooleanConverter : IMultiValueConverter
    {
        ...
        public bool[] CustomConditions { get; set; }
        ...
        {
            if (values == null || Mode==ConverterMode.Unknown) return false;
            var bools = values.OfType<bool>();
            switch (Mode)
            {
                case ConverterMode.All:
                    return bools.All(b => b);
                case ConverterMode.Any:
                    return bools.Any(b => b);
                case ConverterMode.Custom:
                    if (CustomConditions == null) 
                        throw new InvalidOperationException("В режиме Custom необходимо указать свойство CustomConditions");
                    var list = bools as IList<bool> ?? bools.ToList();
                    if (CustomConditions.Length != list.Count)
                        throw new InvalidOperationException(
                            string.Format(
                                "Длина массива CustomConditions ({0}) не совпадает с длиной массива values ({1})",
                                CustomConditions.Length, list.Count));
                    return list.Select((b, i) => CustomConditions[i] == b).All(b => b);
                default:
